...
> The Worker previously had a `GET /accounts` route relaying the Xtream account
> database. It was removed — the frontend serves that list from the same-origin
> Pages Function at `/api/accounts` (`streamvault/functions/api/accounts.js`),
> which sits behind the site's Cloudflare Access login. The Worker route was
> unauthenticated, so anyone who knew the Worker URL could read it.
